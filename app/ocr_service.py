...
class OCRResult:
    ...
    def __init__(self, text: str, confidence: float, engine: str, processing_time: float):
        self.text = text
...
class OCRService:
    ...
    def _initialize_google_vision(self):
        ...
        try:
            if settings.GOOGLE_CLOUD_CREDENTIALS:
                ...
                logger.warning("Google Cloud credentials not provided - Google Vision API unavailable")
        except Exception as e:
            logger.error(f"Failed to initialize Google Vision API: {e}")
            self.google_vision_client = None
    
    def extract_text_from_file(self, file_path: str, use_preprocessing: bool = True) -> OCRResult:
        """
        ...
        try:
            ...
            if file_extension == '.pdf':
                return self._process_pdf_file_smart(file_path, use_preprocessing, start_time)
            elif file_extension in {'.png', '.jpg', '.jpeg', '.tiff', '.bmp'}:
                return self._process_image_file(file_path, use_preprocessing, start_time)
            else:
    ...
    def extract_text_from_bytes(self, file_bytes: bytes, file_extension: str, 
                               use_preprocessing: bool = True) -> OCRResult:
        """
        ...
        try:
            ...
            if file_extension == '.pdf':
                return self._process_pdf_bytes_smart(file_bytes, use_preprocessing, start_time)
            elif file_extension in {'.png', '.jpg', '.jpeg', '.tiff', '.bmp'}:
                return self._process_image_bytes(file_bytes, use_preprocessing, start_time)
            else:
                raise ValueError(f"Unsupported file type: {file_extension}")
    ...
    def _process_pdf_file_smart(self, pdf_path: str, use_preprocessing: bool, start_time: float) -> OCRResult:
        ...
        try:
            ...
            return self._process_pdf_bytes_smart(pdf_bytes, use_preprocessing, start_time)
    ...
    def _process_pdf_bytes_smart(self, pdf_bytes: bytes, use_preprocessing: bool, start_time: float) -> OCRResult:
        """Smart PDF processing: try text extraction first, then OCR if needed."""
        try:
            ...
            try:
                extracted_text, quality_score = self.pdf_converter.extract_text_with_quality_check(pdf_bytes)
                
                if quality_score > 30.0 and len(extracted_text.strip()) > 50:
                    logger.info(f"Using PyMuPDF text extraction (quality: {quality_score:.1f})")
                    processing_time = time.time() - start_time
                    ...
                    logger.info(f"PyMuPDF text quality too low ({quality_score:.1f}), falling back to OCR")
    ...
    def _process_pdf_with_ocr(self, pdf_bytes: bytes, use_preprocessing: bool, start_time: float) -> OCRResult:
    ...
    def _process_image_file(self, image_path: str, use_preprocessing: bool, start_time: float) -> OCRResult:
    ...
    def _process_image_bytes(self, image_bytes: bytes, use_preprocessing: bool, start_time: float) -> OCRResult:
    ...
    def _extract_text_from_image(self, image: np.ndarray, use_preprocessing: bool, 
    ...
    def get_processing_stats(self) -> Dict:
        ...
        return {
            ...
            "processing_engines": ["pymupdf", "tesseract", "google_vision"]
        } 